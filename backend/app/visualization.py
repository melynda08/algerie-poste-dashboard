...
class VisualizationGenerator:
    ...
    # Enhance the __init__ method to better detect date columns
    ...
    # Update the _extract_columns_from_prompt method to improve column detection
    def _extract_columns_from_prompt(self, prompt: str) -> Dict[str, List[str]]:
        ...
        def find_best_column_match(text, available_columns):
            ...
            for col in available_columns:
                ...
                elif re.search(r'\b' + re.escape(col_lower) + r'\b', text):
                    score = len(col_lower) / len(text)  # Longer matches relative to the text get higher scores
                    if score > best_score:
                        best_score = score
                        best_match = col
                
                # Fuzzy match
                elif col_lower in text or text in col_lower:
                    score = 0.5 * len(col_lower) / len(text)  # Partial matches get lower scores
                    if score > best_score:
                        best_score = score
                        best_match = col
        ...
        if 'destination' in prompt_lower or 'dest' in prompt_lower:
            ...
            dest_cols = [col for col in self.df.columns if any(term in col.lower() for term in ['destination', 'dest', 'to', 'arrival', 'target'])]
            if dest_cols:
    ...
    # Update the _detect_chart_type method for better chart detection
    def _detect_chart_type(self, prompt: str) -> str:
        ...
        # If we have a request for count/frequency over time, it's likely a line chart
        if any(term in prompt_lower for term in ['count', 'frequency']) and any(term in prompt_lower for term in ['per month', 'monthly', 'per day', 'daily', 'per week', 'weekly', 'over time']):
            return 'line'
    
    # Update the generate_visualization method to improve chart accuracy
    def generate_visualization(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # If we have a top N request and x column is categorical
            if top_n and columns['x'] and columns['x'][0] in self.categorical_cols:
                logger.info(f"Filtering for top {top_n} values")
                # We need to aggregate the data first
                if not df_agg and len(columns['x']) > 0 and len(columns['y']) > 0:
                    try:
                        ...
                        logger.error(f"Aggregation error: {str(agg_err)}")
                
                # Use aggregated data if available
                if df_agg is not None:
                    # Sort by the y column and take top N
                    df_agg = df_agg.sort_values(by=columns['y'][0], ascending=False).head(top_n)
                    plot_df = df_agg
                    logger.info(f"Filtered to top {top_n} values, data shape: {plot_df.shape}")
            ...
            if chart_type in ['bar', 'bar-grouped', 'bar-stacked', 'pie', 'sunburst']:
                ...
                if len(columns['x']) > 0 and len(columns['y']) > 0:
                    logger.info(f"Performing aggregation for {chart_type} chart")
                    try:
                        # Determine appropriate aggregation function
                        agg_func = 'sum'  # Default
                        if 'average' in prompt.lower() or 'mean' in prompt.lower() or 'avg' in prompt.lower():
            ...
            if len(plot_df) > max_rows:
                logger.info(f"Limiting data from {len(plot_df)} to {max_rows} rows")
            ...
            logger.info(f"Chart columns - x: {columns['x']}, y: {columns['y']}, color: {columns['color']}")
            ...
            if chart_type == 'bar' or chart_type == 'bar-grouped' or chart_type == 'bar-horizontal':
                ...
                if len(columns['y']) > 1 and chart_type != 'bar-horizontal':
                    ...
                    fig = px.bar(
                        ...
                        labels={col: col.replace('_', ' ').title() for col in columns['x'] + columns['y'] + columns['color'] if col in plot_df.columns}
                    )
                else:
                ...
                if len(columns['x']) > 0 and len(columns['y']) > 0:
                    ...
                    if stack_col:
                        ...
                        try:
                            ...
                            logger.info(f"Created stacked bar data with shape: {plot_df.shape}")
                            ...
                            fig = px.bar(
                                ...
                                barmode='stack',
                                labels={
                                    columns['x'][0]: columns['x'][0].replace('_', ' ').title(),
                                    'count': 'Frequency' if agg_func == 'count' else y_col.replace('_', ' ').title(),
                                    stack_col: stack_col.replace('_', ' ').title()
                                }
                            )
                        except Exception as stack_err:
                            logger.error(f"Error creating stacked bar chart: {str(stack_err)}")
                            ...
                            fig = px.bar(
                                ...
                                title=title,
                                barmode='stack',
                                labels={col: col.replace('_', ' ').title() for col in columns['x'] + columns['y'] + [stack_col] if col in plot_df.columns}
                            )
                    else:
                        ...
                        if potential_stack_cols:
                            ...
                            fig = px.bar(
                                ...
                                y=columns['y'][0],
                                title=title,
                                labels={col: col.replace('_', ' ').title() for col in columns['x'] + columns['y'] if col in plot_df.columns}
                            )
                else:
                    ...
                    try:
                        ...
                        if category_col:
                            ...
                            fig = px.line(
                                ...
                                y='count',
                                color=category_col,
                                title=title or f"Count of {category_col} by {period_name}",
                                labels={
                            ...
                            fig = px.line(
                                ...
                                x=period_col,
                                y='count',
                                title=title or f"Count by {period_name}",
                                labels={
                                    period_col: period_name,
                                    'count': 'Count/Frequency'
                                },
                        ...
                        logger.error(f"Error creating time-based line chart: {str(time_err)}")
                ...
                fig = px.scatter(
                    ...
                    title=title,
                    labels={col: col.replace('_', ' ').title() for col in columns['x'] + columns['y'] + columns['color'] + columns['size'] if col in plot_df.columns},
                ...
                if 'trend' in prompt.lower() or 'regression' in prompt.lower() or 'correlation' in prompt.lower():
                    try:
                        ...
                        logger.error(f"Error adding trendline: {str(trend_err)}")
                ...
                if len(columns['x']) > 0 and len(columns['y']) > 0:
                    try:
                        ...
                        fig = px.imshow(
                            pivot_df,
                            title=title,
                            labels=dict(x=columns['y'][0], y=columns['x'][0], color="Value"),
                        ...
                        logger.error(f"Error creating heatmap: {str(heatmap_err)}")
                        ...
                        try:
                            ...
                            logger.error(f"Error creating correlation heatmap: {str(corr_err)}")
            ...
            if fig:
                ...
                logger.error("No figure was created")
