...
class DashboardVisualization(Base):
    __tablename__ = "dashboard_visualization"
    visualization_id = Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id = Column(PG_UUID(as_uuid=True), ForeignKey('app_user.user_id', ondelete='CASCADE'), nullable=False)
    ...
    position = Column(Integer, nullable=True)  # For ordering on dashboard