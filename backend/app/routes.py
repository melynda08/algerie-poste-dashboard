...
CORS(main, resources={r"/*": {"origins": ["http://localhost:3000", "http://localhost:5001"], "supports_credentials": True}})
...
@main.route('/test-cors', methods=['GET'])
def test_cors():
...
# Add a new endpoint to save visualizations to the dashboard
@main.route('/add_visualization_to_dashboard', methods=['POST'])
@login_required
def add_visualization_to_dashboard():
    """Save a visualization to be displayed on the main dashboard"""
    db: Session = next(get_db())
    user_id = get_authenticated_user()
    data = request.get_json()
    
    try:
        # Create a new DashboardVisualization record
        from app.models import DashboardVisualization
        ...
        return jsonify({
            ...
            "visualization_id": str(visualization.visualization_id)
        })
    except Exception as e:
        db.rollback()
        return jsonify({"error": str(e)}), 500

@main.route('/dashboard_visualizations', methods=['GET'])
@login_required
def get_dashboard_visualizations():
    """Get all visualizations saved to the dashboard for the current user"""
    db: Session = next(get_db())
    user_id = get_authenticated_user()
    ...
    try:
        ...
        # Get all visualizations for the user, ordered by created_at (newest first)
        visualizations = db.query(DashboardVisualization).filter(
            DashboardVisualization.user_id == user_id
        ).order_by(DashboardVisualization.created_at.desc()).all()
        ...
        return jsonify([{
            ...
            "prompt": viz.prompt,
            "file_id": viz.file_id,
            "file_name": viz.file_name,
            "created_at": viz.created_at.isoformat(),
            "is_pinned": viz.is_pinned,
            "position": viz.position
        } for viz in visualizations])
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@main.route('/dashboard_visualizations/<visualization_id>', methods=['DELETE'])
@login_required
def delete_dashboard_visualization(visualization_id):
    """Delete a visualization from the dashboard"""
    db: Session = next(get_db())
    user_id = get_authenticated_user()
    ...
    try:
        ...
        visualization = db.query(DashboardVisualization).filter(
            DashboardVisualization.visualization_id == visualization_id,
            DashboardVisualization.user_id == user_id
        ).first()
